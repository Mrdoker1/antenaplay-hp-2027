...
| **Aspect ratio carries meaning** — but by playback context, not genre. 2:3 for anything browsed as a catalogue entry, including the "AntenaPLAY Sport" shelf, which is posters in the Figma file too. 16:9 only for what is playing or has a running time: live and now-on-air, Continue watching, short-form clips. So "Live & Sport" reads as a schedule while "AntenaPLAY Sport" reads as a shelf. | `catalog.ts` (`shape`) |
