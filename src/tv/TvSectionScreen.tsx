...
export function TvSectionScreen({
  ...
  return (
    <div
      className="tv-no-scrollbar absolute inset-0 z-20 overflow-y-auto bg-tv-ground py-[30px]"
      style={{
        paddingLeft: 'calc(var(--tv-rail) + var(--tv-safe) - 26px)',
        paddingRight: 'calc(var(--tv-safe) - 26px)',
      }}
    >
      <h1 className="px-[26px] pt-[8px] text-[40px]/[46px] font-black uppercase tracking-[-0.02em] text-tv-action">
        {title}
      </h1>

      {filters.length > 1 && (
        <div className="tv-no-scrollbar -my-[10px] mt-[14px] flex items-center gap-[20px] overflow-x-auto px-[26px] py-[24px]">
      ...
      <div className="mt-[10px] flex flex-col gap-[26px]">
